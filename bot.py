# ============================================================
# 🤖 نبر وان — كود كامل جاهز لـ Render + تليجرام Webhook
# ============================================================
...
# ============================================================
# 🤖 إعدادات تليجرام — استخدم متغيرات البيئة
# ============================================================
TELEGRAM_BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN", "8792351652:AAGAg4g3E9PB29upyMe_lverCMtelPzvtq8")
TELEGRAM_BOT_ID = os.getenv("TELEGRAM_BOT_ID", "8674500253")
RENDER_URL = os.getenv("RENDER_EXTERNAL_URL", "https://mmm-9es4.onrender.com")  # ← ضع رابطك هنا
...
def جلب_شموع_السعر(الزوج):
    ...
    try:
        ...
        logging.warning("Binance error: %s — using fallback", e)
...
def تحليل_السوق(الزوج, مبلغ, المخاطر):
    ...
    أصوات_شراء = 1 if ema9 > ema21 else 0
    أصوات_شراء += 1 if (50 <= rsi < 75) or (rsi < 25) else 0
    اتجاه = "شراء" إذا أصوات_شراء >= 2 else "بيع"
    نسبة_الثقة = round(75 + abs(ema9 - ema21)/سعر_الدخول*800 + (50 - abs(rsi-50))*0.2, 1)
    نسبة_الثقة = max(50, min(98, نسبة_الثقة))
    قوة_الترند = round(55 + abs(ema9 - ema21)/سعر_الدخول*1500, 0)
    ...
    ربح_أول, ربح_ثاني, ربح_ثالث = round(abs(ربح_أول),2), round(abs(ربح_ثاني),2), round(abs(ربح_ثالث),2)
...
@app.get("/api")
def api_home():
    return {"المنصة": "نبر وان", "الإصدار": "1.2.1", "الحالة": "تعمل", "الواجهة": "/"}
...
async def startup():
    if TELEGRAM_BOT_TOKEN and RENDER_URL:
        telegram_request("setWebhook", {"url": f"{RENDER_URL}/telegram/webhook"})
...
🤖 <b>مرحباً بك في نبر وان TITAN</b>
🆔 معرف البوت: <code>{TELEGRAM_BOT_ID}</code>

أوامر البوت:
/تحليل — تحليل السوق الحالي
/help — عرض المساعدة
        """)
    elif text == "/تحليل":
        try:
            res = تحليل_السوق("XAUUSD", 1000, "متوازن")
            telegram_send(chat_id, f"""
📊 <b>تحليل السوق — {res['الزوج_المالي']}</b>
⏰ {res['التاريخ_والوقت']}
💰 السعر: {res['التحليل']['سعر_السوق_الحالي']}
📈 الاتجاه: {res['التحليل']['اتجاه_السوق']}
🧠 الثقة: {res['التحليل']['نسبة_ثقة_الذكاء_الاصطناعي']}
🎯 الإشارة: <b>{res['النتيجة_النهائية']['الاشارة']}</b>
🔒 وقف الخسارة: {res['وقف_الخسارة']['السعر']}
🎯 الأهداف: {res['الأهداف']['الهدف_الأول']['السعر']} | {res['الأهداف']['الهدف_الثاني']['السعر']} | {res['الأهداف']['الهدف_الثالث']['السعر']}
📊 نسبة R/R: {res['نسبة_الربح_إلى_الخسارة']}
            """)
        except Exception as e:
            telegram_send(chat_id, f"❌ خطأ: {str(e)}")
    elif text == "/help":
        telegram_send(chat_id, "أوامر: /start, /تحليل, /help")
    return {"ok": True}
...
<div class="footer">🤖 نبر وان — منصة التداول بالذكاء الاصطناعي<br>الإصدار 1.2.1 — بيانات حقيقية من Binance · تحليل فني حقيقي + بوت تليجرام</div>
</div>
<script>
